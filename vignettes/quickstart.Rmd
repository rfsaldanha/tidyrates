...
Some packages like the `epitools` compute these adjusted rates. This package functions wraps the `epitools` functions in a tidy way, allowing the computation of age adjusted rates for several groups using key variables, like year and regions by example.
...
For example, let's use the Fleiss dataset quoted by the `epitools ` package (Fleiss, 1981, p. 249 ). 
...
The Fleiss data present events (`count` object) and population (`population` object) for 6 age groups and five different groups (1 to 5+).

The `tidyrates` package present the same Fleiss dataset as a tibble.
...
The Fleiss example present uses the average population as standard reference. 
